...
import { setCartOff } from "../context/actions/displayCardAction";
import {
  BiChevronsRight,
  FcClearFilters,
  TbCurrencyNaira,
          ...
          <>
            <div className="flex flex-col w-full items-start justify-start gap-3 h-[65%] overflow-y-scroll scrollbar-none px-4">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
                  <CartItemsCard key={i} index={i} data={item} />
                ))}
            </div>
            <div className="bg-zinc-800 rounded-t-[40px] w-full h-[35%] flex flex-col items-center justify-center    mb-12 px-4 py-6 gap-10">
              <div className="w-full flex items-center justify-evenly">
                ...
                <p className="text-3xl text-orange-500 font-semibold flex items-center justify-center gap-1">
                  <TbCurrencyNaira className="text-primary" />
                  {total}
                </p>
              </div>
              <motion.button
                ...
                className="bg-orange-400 w-[70%] px-9 mb-12 py-3 text-xl text-headingColor font-semibold hover:bg-orange-500 drop-shadow-md rounded-2xl"
              >
                Check Out
              </motion.button>
            </div>
...
export const CartItemsCard = ({ index, data }) => {
  const cart = useSelector((state) => state.cart);
  const user = useSelector((state) => state.user);
  ...
  const decrementCart = (productId) => {
    console.log("Decrementing cart item with productId:", productId);
  ...
  const incrementCart = (productId) => {
    console.log("Incrementing cart item with productId:", productId);
    ...
    >
      ...
      <div className="flex items-center justify-start gap-1 w-full">
        ...
        <p className="text-sm  flex items-center justify-center gap-1  font-semibold text-red-400 ml-auto">
          <TbCurrencyNaira className="text-red-400" /> {itemTotal}
        </p>
      </div>
